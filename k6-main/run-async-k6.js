...
async function runTestFile(testFile) {
  ...
  const htmlReportPath = path.join(testDir, 'htmlReport.html');

  console.log(`▶ Running ${testFile}...`);
  try {
    await execAsync(`set K6_WEB_DASHBOARD=true&& set K6_WEB_DASHBOARD_EXPORT=${htmlReportPath}&& k6 run ${testFile} --summary-export=${resultPath} --no-color > ${logPath}`);
