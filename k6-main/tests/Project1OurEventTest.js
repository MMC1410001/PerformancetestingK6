...
export default function () {
  const url = 'https://project1.project1.app/api/v1/our-events';
  ...
  check(res, {
    ...
    'response is an array': () => Array.isArray(json),
    'first event has id': () => json.length > 0 && typeof json[0].id === 'string',
    'first event has title': () => json[0]?.title?.length > 0,
    'startTime and endTime present': () => json[0]?.startTime && json[0]?.endTime,
    'URL is valid string': () => typeof json[0]?.url === 'string' && json[0].url.startsWith('http'),
  });

  console.log(`Fetched ${json.length} events. First event title: ${json[0]?.title}`);
}
