...
export default function () {
  const url = 'https://project1.project1.app/api/v1/kpi';
  ...
  check(res, {
    ...
    'vua is a number': () => typeof json.data?.vua === 'number',
    'downloads is a number': () => typeof json.data?.downloads === 'number',
    'ria is a number': () => typeof json.data?.ria === 'number',
    'starUsers is a number': () => typeof json.data?.starUsers === 'number',
    'starUsers is not zero': () => json.data?.starUsers > 0,
  });

  console.log(`KPI: vua=${json.data?.vua}, downloads=${json.data?.downloads}, ria=${json.data?.ria}, starUsers=${json.data?.starUsers}`);
}
