// This file checks the form 16 upload functionality of the PlanMyTax application.
...
export default function () {
  ...
  // 1. Upload PDF File to PlanMyTax
  group("1. Upload PDF to PlanMyTax", function () {
    const formData = {
  ...
  // 2. Send file to OneFin endpoint for tax extraction
  group("2. POST PDF to OneFin Taxation", function () {
    const formData = {
      file: http.file(pdfBinary, "sample.pdf", "application/pdf"),
    };
    ...
    check(taxRes, {
      "OneFin status is 200": (res) => res.status === 200,
      "OneFin response is JSON": (res) =>
        res.headers["Content-Type"].includes("application/json"),
    });
    ...
    try {
      ...
      console.error("Failed to parse OneFin response:", taxRes.body);
      return;
    }
